...
public class HeadDroppingEvent implements Listener {
    ...
    private ItemStack createSkull(String name, String b64) {
        ...
        GameProfile profile = new GameProfile(randomUUID, name);
    ...
    public void onDeath(EntityDeathEvent e) {
        ...
        if (chance < 0 || chance >= ThreadLocalRandom.current().nextDouble(0, 100)) {
            ItemStack head = switch(e.getEntityType()) {
                ...
                case CAT -> {
                    yield switch(((Cat) e.getEntity()).getCatType().getKeyOrNull().toString()) {
                ...
                case CHICKEN -> {
                    yield switch(((Chicken) e.getEntity()).getVariant().getKeyOrNull().toString()) {
                ...
                case COW -> {
                    yield switch(((Cow) e.getEntity()).getVariant().getKeyOrNull().toString()) {
                ...
                case FROG -> {
                    yield switch(((Frog) e.getEntity()).getVariant().getKeyOrNull().toString()) {
                ...
                case PIG ->  {
                    yield switch(((Pig) e.getEntity()).getVariant().getKeyOrNull().toString()) {
                ...
                case WOLF -> {
                    yield switch(((Wolf) e.getEntity()).getVariant().getKeyOrNull().toString()) {
                        ...
                        default -> throw new IllegalStateException("Unexpected value: " + ((Wolf) e.getEntity()).getVariant().getKeyOrNull().toString());
                    };
                }
